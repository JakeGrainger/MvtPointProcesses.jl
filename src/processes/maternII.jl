...
function rand(model::MaternIIProcess)
    ...
    for (m1,p) in zip(marks,parent(proposals)) #  note this could be made more efficient
        if sum(m1 ≤ m2 && norm(p.coords.-q.coords) < model.r for (m2,q) in zip(marks,parent(proposals))) == 1 # one of these is distance to self
            push!(X,p)
        end
    end
